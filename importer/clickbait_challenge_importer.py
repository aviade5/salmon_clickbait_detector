...
from commons.schema_definition import Post, Author, Target_Article, Target_Article_Item
from importer.json_importer_parent import JSON_Importer_Parent
...
class Clickbait_Challenge_Importer(JSON_Importer_Parent):
    ...
    def _read_records_file_and_add_to_db(self):
        ...
        with open(self._json_path + self._records_file_name) as file:
                for line in file:
                    ...
                    target_article = self._parse_target_article(json_content)
                    target_articles.append(target_article)

                    target_article_items = self._parse_target_article_items(json_content, target_article_items)
    ...
    def _get_str_and_date_formats(self, str_timestamp):
        date_timestamp = dateutil.parser.parse(str_timestamp, ignoretz=True)
    ...
    def _parse_target_article(self, json_content):
    ...
    def _parse_target_article_items(self, json_content, target_article_items):

        post_id = json_content["id"]

        target_paragraphs = json_content["targetParagraphs"]
        item_type = u'paragraph'
        target_article_items = self._fill_target_article_items_by_type(post_id, target_paragraphs, item_type, target_article_items)

        target_captions = json_content["targetCaptions"]
        item_type = u'caption'
        target_article_items = self._fill_target_article_items_by_type(post_id, target_captions, item_type,
                                                                       target_article_items)

        return target_article_items

    def _create_target_article_item(self, post_id, item_type, i, paragraph):
    ...
    def _fill_target_article_items_by_type(self, post_id, target_paragraphs, item_type, target_article_items):
        i = 0
        for paragraph in target_paragraphs:
            target_article_item = self._create_target_article_item(post_id, item_type, i, paragraph)
